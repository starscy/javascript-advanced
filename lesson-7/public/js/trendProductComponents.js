Vue.component('trends', {
    data() {
        return {
            catalogUrl: '/catalogData.json',
            products: [],
            filtered: [],
            imgCatalog: '/img/trends/',
        }
    },
    methods: {
        filter(value) {
    ...
    mounted() {
        this.$parent.getJson(`/api/trends`)
            .then(data => {
                for (let el of data) {
    ...
    <ul class="products__list">
            ...
            <trend v-for="item of filtered" :key="item.id_product" :imgCatalog="imgCatalog" :product="item"></trend>
        </ul>
    `
});
