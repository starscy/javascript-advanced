  ...
  methods: {
    ...
    addProduct(item) {
      ...
      if (find) {
        ...
        let productCart = Object.assign(item, { quantity: 1 });
        this.basket.push(productCart);
      }
    ...
    deleteProduct(item) {
      item.quantity--;
      if (item.quantity === 0) {
        //     let index = this.basket.indexOf(item.quantity == 0);
        let find = this.basket.find((product) => product.quantity === 0);
